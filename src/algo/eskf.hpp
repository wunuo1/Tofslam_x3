...
namespace zjloc
{
    ...
    public:
        ...
        struct Options
        {
            ...
            // double gyro_var_ = 1.8293686776900016e-3;      // 陀螺测量标准差
            // double acce_var_ = 1.1149610836356934e-2;      // 加计测量标准差
            // double bias_gyro_var_ = 2.8437858107272050e-5; // 陀螺零偏游走标准差
            // double bias_acce_var_ = 2.7638460656064342e-4; // 加计零偏游走标准差

            /// 里程计参数
            double odom_var_ = 0.1;
            double odom_span_ = 0.1;       // 里程计测量间隔
            // double wheel_radius_ = 0.155;  // 轮子半径
            double wheel_radius_ = 0.065;  // 轮子半径
        ...
        bool ObserveWheelSpeed(const Odompos &odompos);
        bool ObserveWheelPos(const Odompos &odompos);
        ...
        // reloc设置状态X
        void SetRelocX(SO3 R_reloc, VecT p_reloc){
            R_ = R_reloc;
            p_ = p_reloc;
        }
    ...
    bool ESKF<S>::Predict(const IMU &imu){
        ...
        if (dt > (5 * options_.imu_dt_) || dt < 0){
            ...
            //LOG(INFO) << "timestamp1: "<<imu.timestamp_<<" current_time: "<<current_time_ << dt;
    ...
    template <typename S>
    bool ESKF<S>::ObserveWheelSpeed(const Odompos &odompos){
        ...
        //     options_.wheel_radius_ * odom.right_pulse_ / options_.circle_pulse_ * 2 * M_PI / options_.odom_span_;
        // double average_vel = 0.5 * (velo_l + velo_r);
        // double average_vel = odompos.vx_;//std::sqrt(odompos.vx_*odompos.vx_+odompos.vy_*odompos.vy_);
        VecT vel_odom(odompos.vx_, odompos.vy_, odompos.vz_);
        VecT vel_world = R_ * vel_odom;
    ...
    template <typename S>
    bool ESKF<S>::ObserveWheelPos(const Odompos &odompos){
        
        assert(odompos.timestamp_ >= current_time_);
        // odom 修正以及雅可比
        // 使用三维的轮速观测，H为3x18，大部分为零
        Eigen::Matrix<S, 3, 18> H = Eigen::Matrix<S, 3, 18>::Zero();
        ...
        VecT pos_odom = odompos.pos_;
        // VecT vel_world = /*R_ * */vel_odom;
        std::cout<<"p_:"<<p_<<std::endl;
        std::cout<<"odom:"<<pos_odom<<std::endl;
        dx_ = K * (pos_odom - p_);
