...
namespace zjloc
{
     ...
     OdometryFrame::OdometryFrame(pcl::PointCloud<pcl::PointXYZI> raw, Eigen::Isometry3d pose, double stamp, cv::Mat img) : pose(pose), timestamp(stamp), img_(img)
     {
          raw_cloud.reset(new pcl::PointCloud<pcl::PointXYZI>());
          raw_cloud = raw.makeShared();
          cloud_ = raw_cloud;
     }

     OdometryFrame::OdometryFrame(pcl::PointCloud<pcl::PointXYZI> raw, Eigen::Isometry3d pose, cv::Mat img) : pose(pose), downsample_resolution(0.1f), img_(img)
     {
          raw_cloud.reset(new pcl::PointCloud<pcl::PointXYZI>());
          raw_cloud = raw.makeShared();
          cloud_ = raw_cloud;
     ...
     const pcl::PointCloud<pcl::PointXYZI>::Ptr& OdometryFrame::cloud(float downsample_resolution) 
     {
