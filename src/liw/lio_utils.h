...
namespace zjloc
{
     ...
     struct MeasureGroup
     {
          ...
          Eigen::VectorXf lidar_coeff_;
          cv::Mat img_;
          std::deque<IMUPtr> imu_;      // 上一时时刻到现在的IMU读数
          rclcpp::Time lidar_begin_rostime_ = rclcpp::Time(0);
          OdomposPtr odom_;
     };
     ...
     EIGEN_MAKE_ALIGNED_OPERATOR_NEW
          ...
          OdometryFrame(pcl::PointCloud<pcl::PointXYZI> raw, Eigen::Isometry3d pose, double stamp, cv::Mat img);

          OdometryFrame(pcl::PointCloud<pcl::PointXYZI> raw, Eigen::Isometry3d pose, cv::Mat img);
