...
public: 
    ...
    std::map<int, float> detectDistanceLoopClosureID(int id);
    std::pair<int, float> detectDatabaseLoopClosureID( void ); // int: nearest node index, float: relative yaw 

    void makeAndSaveScancontextAndPose( pcl::PointCloud<SCPointType> & _scan_down, std::vector<double> & frame_pose);
    std::pair<int, std::vector<double>> detectDatabaseLoopClosureIDAndPose( void ); // int: nearest node index, float: relative yaw
...
public:
    ...
    std::vector<std::vector<double>> pose_database_;
    std::vector<pcl::PointCloud<SCPointType>> cloud_database_;
